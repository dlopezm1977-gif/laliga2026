...
function incHome(inc) { return inc.is_home ?? (inc.team === 'home'); }
function fmtMin(inc) {
...
function IncidentsTab({ data, detail }) {
  if (!data) return null;
...
function FormationPitch({ players, formation }) {
  ...
  const dots = rows.flatMap(({ players: rowPlayers, y }) =>
    rowPlayers.map((player, i, arr) => ({
      player, x: (80 / (arr.length + 1)) * (i + 1), y,
...
function PlayerRow({ p, dimmed }) {
  return (
    <div className="md-lineup-player" style={dimmed ? { opacity: .65 } : undefined}>
      ...
      {p.position && (
        <span className={`md-lineup-pos md-lineup-pos--${p.position.toLowerCase()}`}>
...
export default function MatchDetailModalLaliga({ match, detail, stats, lineups, incidents, loading, error, onClose }) {
  ...
  // Nombres para cabecera: FD short names (tienen crest); score de bzzoiro
  const homeTeamLabel = match?.homeTeam ?? detail?.home_team ?? 'Local';
  const awayTeamLabel = match?.awayTeam ?? detail?.away_team ?? 'Visitante';
          ...
          <>
            <div className="md-header">
              <div className="md-team">
                <img className="md-crest" src={crestUrl(homeTeamLabel)} alt="" />
                <span className="md-team-name">{homeTeamLabel}</span>
              </div>
                ...
                {(isFinished || isLive) ? (
                  <>
                    <div className="md-score-main">
                      <span>{detail?.home_score ?? match?.homeScore ?? '–'}</span>
                      <span className="md-sep">-</span>
                      <span>{detail?.away_score ?? match?.awayScore ?? '–'}</span>
                    </div>
                    {hasHT && (
              ...
              <div className="md-team">
                <img className="md-crest" src={crestUrl(awayTeamLabel)} alt="" />
                <span className="md-team-name">{awayTeamLabel}</span>
              </div>
            </div>

            <div className="md-meta">
              <span>{formatDateLong(detail?.event_date ?? match?.utcDate)}</span>
              ...
              <>
                {highlight && (
                  <div className="md-section">
                    ...
                    <a href={highlight.url} target="_blank" rel="noopener noreferrer" className="md-highlight-link">
                      {highlight.thumbnail && (
            ...
            {activeTab === 'alineaciones' && <LineupsTab  data={lineups}   />}
            {activeTab === 'incidencias'  && <IncidentsTab data={incidents} detail={detail} />}
          </>
        )}
      </div>
    </div>
